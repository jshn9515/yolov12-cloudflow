...
```bash
query$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
query$ sam local start-api
query$ curl http://localhost:3000/
```
...
```bash
query$ sam logs -n HelloWorldFunction --stack-name "query" --tail
```
...
```bash
query$ pip install pytest pytest-mock --user
query$ python -m pytest tests/ -v
```
